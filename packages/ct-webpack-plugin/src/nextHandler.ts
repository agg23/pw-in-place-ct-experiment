...
import type { PresetHandlerResult, WebpackDevServerConfig } from './types';
import { getMajorVersion, ModuleClass, SourcedDependency, SourcedWebpack, sourceFramework, sourceWebpackDevServer } from './sourceRelativeWebpackModules'
...
function getNextJsPackages (devServerConfig: WebpackDevServerConfig) {
  const resolvePaths = { paths: [devServerConfig.projectRoot] }
  const packages = {} as {
...
async function loadWebpackConfig (devServerConfig: WebpackDevServerConfig): Promise<Configuration> {
  ...
  const nextConfig = await loadConfig('development', devServerConfig.projectRoot)
  const runWebpackSpan = getRunWebpackSpan(devServerConfig)
  const reactVersion = getReactVersion(devServerConfig.projectRoot)
  const jsConfigResult = await nextLoadJsConfig?.(devServerConfig.projectRoot, nextConfig)
  const supportedBrowsers = await getSupportedBrowsers(devServerConfig.projectRoot, true, nextConfig)

  const webpackConfig = await getNextJsBaseWebpackConfig(
    devServerConfig.projectRoot,
    {
      buildId: `@cypress/react-${Math.random().toString()}`,
      config: nextConfig,
      dev: true,
      pagesDir: await findPagesDir(devServerConfig.projectRoot),
      entrypoints: {},
...
function getRunWebpackSpan (devServerConfig: WebpackDevServerConfig): { runWebpackSpan?: any } {
  ...
  try {
    try {
      const traceImportPath = require.resolve('next/dist/telemetry/trace/trace', { paths: [devServerConfig.projectRoot] })
...
function sourceNextWebpack (devServerConfig: WebpackDevServerConfig, framework: SourcedDependency) {
  ...
  try {
    const nextConfig = require(path.resolve(devServerConfig.projectRoot, 'next.config.js'))
